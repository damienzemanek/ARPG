...
public class ProTimerTestSuite
{
    ...
    public void Test1_TimerInitialization()
    {
        ...
        var timerEvent = TimerEvent.NoData(
            new TriggerCtx { time = 0, triggerTime = 5.0f }, 
            SomeTriggerLogic.TriggeredOperation
        );
    ...
    public void Test3_TickDeltaTimeAndTrigger()
    {
        ...
        try {
            ...
            var timerEvent = TimerEvent.NoData(
                new TriggerCtx { time = 0, triggerTime = 1.0f }, 
                SomeTriggerLogic.TriggeredOperation
            );
            ...
            Debug.Log("[DEBUG_LOG] Ticked 0.5s, is now at: " + timer.events[0].Ctx.time);
            ...
            Debug.Log($"[DEBUG_LOG] Ticked 0.6s, is now at: {timer.events[0].Ctx.time}. Triggered={SomeTriggerLogic._eventTriggered}");
    ...
    public void Test4_MultiEventSequentialTriggering()
    {
        ...
        var eventB = TimerEvent.NoData(   
            new TriggerCtx { time = 0, triggerTime = 2.0f },
            SomeTriggerLogic.TriggeredOperation
            );
        ...
        Debug.Log("Current trigger count: " + SomeTriggerLogic._triggerCount + "");
    ...
    public void Test5_CountdownLogic()
    {
        ...
        var timerEvent = TimerEvent.NoData(
            new TriggerCtx { time = 5.0f, triggerTime = 0.0f },
            SomeTriggerLogic.TriggeredOperation
        );
        events.Allocate(ref timerEvent);
    ...
    public void Test7_ImmediateTrigger()
    {
        ...
        var timerEvent = TimerEvent.NoData(
            new TriggerCtx { time = 0, triggerTime = 0.0f },
            SomeTriggerLogic.TriggeredOperation
        );
        events.Allocate(ref timerEvent);
    ...
    public void Test9_DataStabilityAndIDReuse()
    {
        for (int i = 0; i < 10; i++)
        {
            ...
            var timerEvent = TimerEvent.NoData(
                new TriggerCtx { time = 0, triggerTime = 0.1f },
                SomeTriggerLogic.TriggeredOperation
            );
            events.Allocate(ref timerEvent);
            ProTimer timer = new ProTimer(TickMath.Add, ref events);
    ...
    public unsafe void Test13_TriggeredDataPtrMutation()
    {
        ...
        var timerEvent = TimerEvent.WithData(
            new TriggerCtx { time = 0, triggerTime = 0.5f },
            SomeTriggerLogic.MutateDataOperation,
            (IntPtr)(&testData)
        );
    ...
    public void Test14_NegativeDeltaTimeHandling()
    {
        ...
        Assert.Less(timer.events[0].Ctx.time, 0);
